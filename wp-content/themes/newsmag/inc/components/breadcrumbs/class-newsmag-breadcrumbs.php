...
class Newsmag_Breadcrumbs {
	...
	public function __construct() {
		...
		$args     = apply_filters( 'newsmag_breadcrumbs_defaults', $defaults );
	...
	private function wrap_breadcrumbs() {
		$this->html_markup = sprintf( '<div class="newsmag-breadcrumbs">%s</div>', $this->html_markup );
	}
	...
	private function get_breadcrumb_prefix() {
		...
		if ( ! is_front_page() ) {
			...
			if ( $this->home_prefix ) {
				$prefix = sprintf( '<span class="newsmag-breadcrumb-prefix">%s:</span>', esc_html( $this->home_prefix ) );
			}
		}
